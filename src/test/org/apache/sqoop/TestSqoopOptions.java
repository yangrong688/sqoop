...
public class TestSqoopOptions {
  ...
  private Set<String> excludedFieldsFromReferencedObjects = new HashSet<>();
  ...
  public void setUp() {
    ...
    excludedFieldsFromReferencedObjects.add("allowNullValueProperties");
  }
  ...
  private void setFieldToValueIfApplicable(Object target, Field field) throws Exception {
    ...
    if(!Modifier.isFinal(modifiers) && !Modifier.isAbstract(modifiers) && !Modifier.isStatic(modifiers)
            && !excludedFieldsFromReferencedObjects.contains(field.getName())) {
      field.setAccessible(true);
